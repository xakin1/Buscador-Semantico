    ...
    for(let datos of jsonComandos){
        ...
        if(datos.commandID == path[4]){
          ...
          for(let step of datos.steps){
            ...
            var stepSinEspacio= step.stepName.replace(/\s/g, '');
            liStep.attr("id",stepSinEspacio);
            a.attr("href", window.location.href+"/step/"+step.stepID);
            a.attr("onclick","cargarDatos("+step.stepID+",'"+step.stepName+"')");
            a.addClass("nav-link");
            i.addClass("link-icon");
            i.attr("data-feather","box");
            span.addClass("link-title");
            var stepCapitalizado=(step.stepName).charAt(0).toUpperCase()+(step.stepName.slice(1));
...
function showModalPopUp(id){
  window.location =  window.location.href+"/step/"+id
}
  ...
  function getCookie(name) {
   ...
   function send_edit_command(name,commandId,description) {
    var type = '0';
    var data
    description != undefined ? data = {"apiKey":"02d5214506fa468484e962868800395f","commandID":commandId+"","description":description,"name":name,"profileID":"1268900770","userPwd":"69637182"}
...
function modificar_nombre(comando){
